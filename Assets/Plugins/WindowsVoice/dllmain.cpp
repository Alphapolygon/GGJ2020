...
*/
    ...
    while (!shouldTerminate)
    {
      ...
      else
      {
        ...
        if (!theSpeechQueue.empty())
        {
		  ...
		  bool gender = theGenderQueue.front();
          theSpeechQueue.pop_front();
		  theGenderQueue.pop_front();
          theMutex.unlock();
  ...
  void addToSpeechQueue(const char* text, bool male)
  {
    if (text)
    {
  ...
  void destroySpeech()
  {
	...
	theGenderQueue.clear();
	theGenderQueue = nullptr;
	delete theGenderQueue;
    CoUninitialize();
